...
class RNN(object):
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			...
			x_input = make_onehot(x[t], self.vocab_size)
			self.deltaV += np.outer(WTL * net_in_p, x_input)
			self.deltaU += np.outer(WTL * net_in_p, s[t - 1])
			self.deltaW += np.outer(W1_loss, s[t])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		no return values
		'''
		net_out_p = np.ones(len(x))
		net_in_p = np.array([s_t * (np.ones(len(s_t)) - s_t) for s_t in s])

		for t in reversed(range(len(x))):
			##########################
			# --- your code here --- #
			##########################
			desire_value = make_onehot(d[t],self.vocab_size)
			loss_out_p = (desire_value - y[t]) * net_out_p[t]
			self.deltaW += np.outer(loss_out_p, s[t])
			loss_in_p = np.zeros((len(x), self.hidden_dims))
			for tt in range(0, steps+1):
				if t - tt < 0:
					break
				if tt == 0:
					loss_in_p[t - tt] = self.W.T.dot(loss_out_p) * net_in_p[t]
				else:
					loss_in_p[t - tt] = self.U.T.dot(loss_in_p[t - tt + 1]) * net_in_p[t - tt]
				self.deltaV += np.outer(loss_in_p[t - tt], make_onehot(x[t - tt],self.vocab_size))
				self.deltaU += np.outer(loss_in_p[t - tt], s[t - tt-1])
	...
	def generate_sequence(self, start, end, maxLength):
		...
		while True:
			...
			word_loss = -np.log(y[-1][word_index])
			loss += word_loss
	...
	def generate_sequence_not_unknown(self, start, end, maxLength):
		'''
		generate a new sequence, using the RNN
		...
		while True:
			y, s = self.predict(x)
			word_index = multinomial_sample_not_unknown(y[-1])
			x.append([word_index])
			sequence.append([word_index])
			word_loss = -np.log(y[-1][word_index])
			loss += word_loss
...
if __name__ == "__main__":
	...
	if mode == "estimate":
		...
		for gg in range(0,6):
			...
			pen = open('results_of_estimate.txt', 'a')
			for g in dictt.keys():
				pen.write(g)
				pen.write('\t')
	...
	if mode == "train":
		...
		X_dev_total = X_dev.copy()
		D_dev_total = D_dev.copy()
		X_dev = X_dev[:dev_size]
		D_dev = D_dev[:dev_size]
		...
		run_loss = 0.
		adjusted_loss = 0.
		run_loss = R.train(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, epochs=10, learning_rate=lr, back_steps=lookback,batch_size=1)
		###########evaluate the model through full-size development set########################
		mean_loss_evaluated_by_total_X_and_D = R.compute_mean_loss(X_dev_total, D_dev_total)
		run_loss = mean_loss_evaluated_by_total_X_and_D
		#######################################################################################
		fraqloss = fraq_loss(vocab, word_to_num, vocab_size)
		adjusted_loss = adjust_loss(run_loss,fracloss=fraqloss,q=q,mode='basic')
		adjusted_loss_not_basic = adjust_loss(run_loss, fracloss=fraqloss, q=q, mode='a')
		np.save(data_folder+"/rnn.U.npy", R.U)
		np.save(data_folder+"/rnn.V.npy", R.V)
		np.save(data_folder+"/rnn.W.npy", R.W)
		print 'mean_loss',run_loss
		print "Unadjusted: %.03f" % np.exp(run_loss)
		print "Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss)
		print "Adjusted_not_basic for missing vocab: %.03f" % np.exp(adjusted_loss_not_basic)
	...
	if mode == "generate":
		...
		start = [word_to_num['<s>']]
		sentence1,loss = r.generate_sequence(start=start,end=end,maxLength=maxLength)
		sentence2, loss2 = r.generate_sequence(start=start, end=end, maxLength=maxLength)
		sentence3, loss3 = r.generate_sequence(start=start, end=end, maxLength=maxLength)
		...
		s3=[]
		for qq in sentence1:
			# print num_to_word[qq[0]]
			s1.append(num_to_word[qq[0]])
			file1.write(str(num_to_word[qq[0]]))
			file1.write('\t')
		file1.write('\t')
		file1.write(str(loss))
		file1.write('\t')
		file1.write(str(np.exp(loss)))
		for qqq in sentence2:
			s2.append(num_to_word[qqq[0]])
			file2.write(str(num_to_word[qqq[0]]))
			file2.write('\t')
		file2.write('\t')
		file2.write(str(loss2))
		file2.write('\t')
		file2.write(str(np.exp(loss2)))
		for qqqq in sentence3:
			s3.append(num_to_word[qqqq[0]])
			file3.write(str(num_to_word[qqqq[0]]))
			file3.write('\t')
		file3.write('\t')
		file3.write(str(loss3))
		file3.write('\t')
		file3.write(str(np.exp(loss3)))
		print loss,' ',loss2,' ',loss3
		print np.exp(loss),np.exp(loss2),np.exp(loss3)
		print s1, ' meanloss: ',loss
		print '\n'
		print s2, 'meanloss: ',loss2
		print '\n'
		print s3, 'meanloss: ',loss3
############################generating sentences after eliminating the influence of unknown words#####################################
	if mode == "generate_without_unknown":
		'''
		starter code for sequence generation
		'''
		data_folder = sys.argv[2]
		...
		start = [word_to_num['<s>']]
		sentence1,loss = r.generate_sequence_not_unknown(start=start,end=end,maxLength=maxLength)
		sentence2, loss2 = r.generate_sequence_not_unknown(start=start, end=end, maxLength=maxLength)
		sentence3, loss3 = r.generate_sequence_not_unknown(start=start, end=end, maxLength=maxLength)
		file1 = open("se1_without_unknown.txt",'a')
		file2 = open('se2_without_unknown.txt','a')
		file3 = open('se3_without_unknown.txt','a')
		s1=[]
		s2=[]
		s3=[]
		for qq in sentence1:
