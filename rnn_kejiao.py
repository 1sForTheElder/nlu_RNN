...
class RNN(object):
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			...
			self.deltaV += np.outer(WTL * s_net_new, inputt)
			self.deltaU += np.outer(WTL * s_net_new, s[t - 1])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			for tau in range(0, 1 + steps):
				...
				self.deltaV += np.outer(delta_in[t - tau], make_onehot(x[t - tau],self.vocab_size))
				self.deltaU += np.outer(delta_in[t - tau], s[t - tau-1])
	...
	def generate_sequence(self, start, end, maxLength):
		...
		while True:
			...
			# if word_index == 14 or word_index == 37 or word_index == 42 or word_index == 148 or word_index == 164 or word_index == 54:
			# 	print len(y[-1])
			# 	print y[-1]
			# 	break
			x.append([word_index])
...
if __name__ == "__main__":
	...
	if mode == "estimate":
		...
		for gg in range(0,3):
			...
			for hh in hidmm:
				for bb in backsteps:
					for ll in learningratee:
						...
						dictt[dict_name] = r.train(X=X, D=D, X_dev=X_dev, D_dev=D_dev, epochs=epochh, learning_rate=ll,
												   back_steps=bb)
			...
			pen = open('including_4_result.txt', 'a')
			for g in dictt.keys():
				pen.write(g)
				pen.write('\t')
	...
	if mode == "train":
		...
		run_loss = 0.
		adjusted_loss = 0.
		run_loss = R.train(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, epochs=10, learning_rate=lr, back_steps=lookback,batch_size=100)
		...
		np.save("data/rnn.Ufor4batch100.npy", R.U)
		np.save("data/rnn.Vfor4batch100.npy", R.V)
		np.save("data/rnn.Wfor4batch100.npy", R.W)
	...
	if mode == "generate":
		...
		U,V,W = np.load(rnn_folder + "/rnn.Ufor2batch100.npy"), np.load(rnn_folder + "/rnn.Vfor2batch100.npy"), np.load(rnn_folder + "/rnn.Wfor2batch100.npy")
		vocab_size = len(V[0])
		hdim = len(U[0])
		...
		start = [word_to_num['<s>']]
		sequence,loss = r.generate_sequence_not_unknown(start=start,end=end,maxLength=maxLength)
		sequence2, loss2 = r.generate_sequence_not_unknown(start=start, end=end, maxLength=maxLength)
		sequence3, loss3 = r.generate_sequence_not_unknown(start=start, end=end, maxLength=maxLength)
